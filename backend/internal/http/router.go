...
func NewRouter(deps Dependencies) http.Handler {
	...
	mux.Handle("POST /api/v1/mutes", handleCreateMute(deps))
	mux.Handle("DELETE /api/v1/mutes", handleDeleteMute(deps))
	mux.Handle("GET /api/v1/mutes", handleListMutes(deps))
	mux.Handle("POST /api/v1/blocks", handleCreateBlock(deps))
	mux.Handle("DELETE /api/v1/blocks", handleDeleteBlock(deps))
	mux.Handle("GET /api/v1/blocks", handleListBlocks(deps))
	mux.Handle("GET /api/v1/bookmarks", handleListBookmarks(deps))
	mux.Handle("POST /api/v1/notes/{id}/quote", handleCreateQuotePost(deps))
	mux.Handle("POST /api/v1/notes/{id}/bookmark", handleCreateBookmark(deps))
	mux.Handle("DELETE /api/v1/notes/{id}/bookmark", handleDeleteBookmark(deps))
